    ...
    <thead>
        <tr>
            <form id="myform">
                <th>STT</th>
            <th scope="col">Name
                <a href="?sort=product_name&by=desc" class="linkDesc" id=''><i class="fa-solid fa-arrow-down"></i></a>
                <a href="?sort=product_name&by=asc" class="linkAsc" id=''><i class="fa-solid fa-arrow-up"></i></a>
            </th>
            <th>
                Giá Bán
                <a href="?sort=product_price_new&by=desc" class="linkPriceDesc" id=''><i class="fa-solid fa-arrow-down"></i></a>
                <a href="?sort=product_price_new&by=asc" class="linkPriceAsc" id=''><i class="fa-solid fa-arrow-up"></i></a>
            </th>
...
const dataTbody = document.querySelector('#dataTbody')
    ...
    linkPriceDesc.addEventListener("click", function(event) {
        const url = `https://quocmanh.com/Laravel-8/api/demo/restfulapi/product/show?sort=product_price_new&by=`;
        t = 0;
        dataTbody.innerHTML = '';
        event.preventDefault()
        let sortName = getParameterByName('sort', url)
        ...
        "Access-Control-Allow-Origin": "*",
            ...
            .then((data) => {
                // window.location.reload();
                data.data.forEach(element => {
                    let  price = element.product_price_new
                    let priceNew = `{{  currency_format(${element.product_price_new}, 'đ') }}`;
                    t++;
                    dataTbody.innerHTML +=
                    `<tr>
                <th scope="row">${t} </th>
    ...
    linkPriceAsc.addEventListener("click", function(event) {
        ...
        "Access-Control-Allow-Origin": "*",
            ...
            .then((data) => {
                ...
                data.data.forEach(element => {
                    let  price = element.product_price_new
                    let priceNew = `{{  currency_format(${pelement.product_price_new}, 'đ') }}`;
                    t++;
                    dataTbody.innerHTML +=
                    `<tr>
                <th scope="row">${t} </th>
