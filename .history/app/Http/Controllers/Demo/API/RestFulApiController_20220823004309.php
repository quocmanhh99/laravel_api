...
class RestFulApiController extends Controller
{
    ...
    {
        $data = DemoProduct::orderBy('product_name', 'desc')->simplePaginate(5);
    ...
    public function searchProduct(Request $request){
        ...
        $products = DemoProduct::where('product_name', 'like', "%{$search}%")->get();
        return response()->json([
            'data' => $products,
            'status_code' => 200,
            'message' => 'Hiển thị dữ liệu thành công',
    ...
    function sortProduct(Request $request){
        ...
        if($sortName == 'product_name'){
            $sort = ['desc','asc'];
            if(!empty($by) && in_array($by,$sort)){
                if($by == 'desc'){
                    $by = 'asc';
                    $products = DemoProduct::orderBy('product_name', 'asc')->get();
                }else{
                    $by = 'desc';
            ...
            if(!empty($by) && in_array($by,$sort)){
                if($by == 'desc'){
                    $by = 'asc';
                    $products = DemoProduct::orderBy('updated_at', 'asc')->get();
                    // return $products->toSql();
                }else{
                    $by = 'desc';
                    $products = DemoProduct::orderBy('updated_at', 'desc')->get();
                }
            }else{
                $by = 'asc';
                $products = DemoProduct::orderBy('updated_at', 'desc')->get();
    ...
    {
        $by = 'asc';
        $products = DemoProduct::orderBy('product_name', 'desc')->simplePaginate(5);
        return view('demo.restfulapi.product.ajax', compact('products','by'));
