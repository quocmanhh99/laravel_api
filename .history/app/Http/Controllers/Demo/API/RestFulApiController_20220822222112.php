    ...
    function sortProduct(Request $request){
        ...
        if($sortName == 'product_name'){
            ...
            if(!empty($by) && in_array($by,$sort)){
                if($by == 'desc'){
                    $by = 'asc';
                    $products = DemoProduct::orderBy('created_at', 'asc')->get();
                    // return $products->toSql();
                }else{
                    $by = 'desc';
                    $products = DemoProduct::orderBy('created_at', 'desc')->get();
                }
            }else{
                $by = 'asc';
                $products = DemoProduct::orderBy('product_price_new', 'desc')->get();
            }
        }
        return response()->json([
            'data' => $products,
